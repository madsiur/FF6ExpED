...
namespace FF6exped.Misc
{
    public static class ByteUtils
    {
        ...
        public static byte[] ReadBytes(String fileName, int offset, int numBytes)
        {
        ...
        {
            MessageBox.Show(
                "Error accessing data at $" + offset + " in " + length + " byte array", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
        }
        ...
        public static void SetByte(ref byte[] data, int offset, byte set)
        {
            try
            {
                data[offset] = set;
        ...
        public static void SetShort(ref byte[] data, int offset, int set)
        {
            try
            {
                data[offset] = (byte)(set & 0xff);
        ...
        public static void SetEmptyBlock(ref byte[] src, int offset, int length, byte value)
        {
            for (int i = offset; i < length; i++)
                src[i] = value;
        }
